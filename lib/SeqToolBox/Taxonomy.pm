...
sub new {
	...
	unless ( -s $gi_taxid_db || -s $nodes_db ) {
		croak 'Taxonomy databases not found. Did you run "update_taxonomy.pl"?';
	}
	...
	$self->{names_db_taxid_by_sc_name_statement} =
		$self->{names_db_handle}->prepare ("select tax_id from names where (name=? and name_class='scientific name')");
	my %predefined = (
		"vertebrates" => 7742,
		"fungi"       => 4751,
...
##							{ AutoCommit => 0, RaiseError => 1 } );
	...
	while (1) {
		...
		if ( exists $self->{toplevels}->{$taxid} ) {
#			print STDERR "toplevel reached\n";
#			if (exists $self->{c_cache}->{$taxid}->{$rank}) {
#				print STDERR "$taxid not found\n";
#				return $self->{c_cache}->{$taxid}->{$rank};
#			}else {
...
#			}
		...
		croak "more than one parent found for $taxid\n" if $count > 1;
		#croak "No temp rank" unless $temp_rank;
		#print STDERR "$temp_rank\n";
		if ( $rank =~ /^$temp_rank$/i ) {
			$class = $taxid;
			last;
		}
...
sub get_name {
	my ($self,$tax_id) = @_;
	$self->{names_db_statement}->execute($tax_id);
	my @row = $self->{names_db_statement}->fetchrow_array();
...
sub get_taxid_by_name {
	my ($self, $name) = @_;
	$self->{names_db_taxid_by_sc_name_statement}->execute($name);
	my @row = $self->{names_db_taxid_by_sc_name_statement}->fetchrow_array();
	if (scalar (@row) == 1) {
		return $row[0];
	}else {
		return @row;
